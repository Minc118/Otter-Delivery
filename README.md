...
- `driver-service` in Go with Supabase PostgreSQL-backed driver availability, position, assignment, route estimate, and tracking APIs.
...
- Supabase PostgreSQL for the Driver Service

No real AI, maps, translation, payment, or auth APIs are connected in the current MVP.
...
Set `DRIVER_DATABASE_URL` in `.env` before starting the service:

```bash
DRIVER_DATABASE_URL=postgresql://postgres:<password>@db.rotymfjrtkncxixmiqdr.supabase.co:5432/postgres?sslmode=require
```

Run migrations with Supabase CLI from the repository root:
...
- 订单确认页和模拟支付方式选择。
- 下单成功页。
- 订单追踪页，Driver Service 可提供数据库中的追踪数据。
- 餐厅排行榜页和 AI Picks 区域。
- 登录页，为后续认证预留。
...
- `driver-service` 使用 Go 实现，提供基于 Supabase PostgreSQL 的 driver、位置、派单、路线估算和订单追踪 API。
...
- Driver Service 使用 Supabase PostgreSQL
...
先在 `.env` 中设置 `DRIVER_DATABASE_URL`，并按顺序运行 `backend/driver-service/migrations` 目录中的 SQL migration。
...
- 仅桌面端 MVP。
- 前端仍以 mock data 为主，Driver Service 已接入 Supabase PostgreSQL。
- 不实现真实支付。
- 不实现真实认证。
- 路线 ETA 仍使用 mock provider interface，后续可替换为 Google Maps 或 Google Routes。
- Gemini、Google Translation、Google Maps、Keycloak 等均为后续集成方向。
